...
#ifndef INVDebugLogResponse
#define INVDebugLogResponse(response, responseObject,request, error) [[INVLogger sharedLogger] logResponse:response responseObject:responseObject request:request error:error];
#endif
 ...
 */
#ifndef INVDebugLog
#define INVDebugLog(format, ...) [[INVLogger sharedLogger] debugLog:[NSString stringWithFormat:@"%s", __PRETTY_FUNCTION__] message: format, ##__VA_ARGS__];
#endif

@interface INVLogger : NSObject

+(instancetype)sharedLogger;

/**
 * Enabled switch, default NO - default set by INVConfiguration debug property
 */
