#import "INVApiClient.h"

NSString *const INVResponseObjectErrorKey = @"INVResponseObject";
...
static NSDictionary * INV__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * INV__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = INV__headerFieldsForResponse(response);
...
@interface INVApiClient ()

@property (nonatomic, strong) NSDictionary* HTTPResponseHeaders;

@end

@implementation INVApiClient

- (instancetype)init {
    NSString *baseUrl = [[INVConfiguration sharedConfig] host];
...
- (instancetype)initWithBaseURL:(NSURL *)url {
    ...
    if (self) {
        ...
        self.responseDeserializer = [[INVResponseDeserializer alloc] init];
        self.sanitizer = [[INVSanitizer alloc] init];
...
+(NSNumber*) queueRequest {
    ...
    INVDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        INVDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    } else {
        return NO;
...
- (void) configureCacheReachibility {
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        reachabilityStatus = status;
        INVDebugLog(@"reachability changed to %@",AFStringFromNetworkReachabilityStatus(status));
        [INVApiClient setOfflineState:status == AFNetworkReachabilityStatusUnknown || status == AFNetworkReachabilityStatusNotReachable];
...
- (void) operationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        INVDebugLogResponse(response, responseObject,request,error);
        strongSelf.HTTPResponseHeaders = INV__headerFieldsForResponse(response);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[INVResponseObjectErrorKey] = responseObject;
        }
...
- (void) downloadOperationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString * filename = INV__fileNameForResponse(response);
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [INVJSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [INVJSONResponseSerializer serializer];
    } else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = INVPercentEscapedStringFromString(safeString);
    ...
    if (offlineState) {
        INVDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        INVDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        INVDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [INVApiClient queueRequest];
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = INVPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, INVPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[INVQueryParamCollection class]]){
            INVQueryParamCollection * coll = (INVQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = INVPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = INVPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = INVPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (AFSecurityPolicy *) customSecurityPolicy {
    ...
    INVConfiguration *config = [self configuration];
...
- (INVConfiguration*) configuration {
    return [INVConfiguration sharedConfig];
}
